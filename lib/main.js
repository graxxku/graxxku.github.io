    ...
    for(let i=0; i<60; i++){
        ...
        let imu = await ble.readValue().then(dataview => {
            ...
            for (let i = 0; i < numSamples; i += samplesPerSet) {
                const acc_x = toSigned16Bit(buffer[1], buffer[0]);
                const acc_y = toSigned16Bit(buffer[3], buffer[2]);
                const acc_z = toSigned16Bit(buffer[5], buffer[4]);
            
                const gyro_x = toSigned16Bit(buffer[7], buffer[6]);
                const gyro_y = toSigned16Bit(buffer[9], buffer[8]);
                const gyro_z = toSigned16Bit(buffer[11], buffer[10]);
